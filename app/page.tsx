    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative z-10 px-5 py-9 sm:px-7 lg:px-8 lg:py-11">
        <div className="max-w-[565px]">
          ...
          <h1 className="mt-5 text-[29px] font-extrabold leading-[1.1] tracking-normal text-white sm:text-[40px] lg:text-[43px]">
            Выводим бизнес
            <br />в топ поисковых систем
            <br />
            <span className="text-[#2d72ff]">и увеличиваем прибыль</span>
          </h1>
          <p className="mt-4 max-w-[500px] text-[14px] leading-7 text-slate-300">
            Комплексное SEO-продвижение, которое приводит целевой трафик и превращает его в клиентов.
          </p>
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <Link
              href={pathFor('/contacts', locale)}
              className="inline-flex h-11 w-full max-w-[340px] items-center justify-center rounded-md bg-[#2368ff] px-6 text-[13px] font-extrabold text-white shadow-[0_12px_32px_rgba(35,104,255,0.44)] sm:w-auto"
            >
              Получить бесплатный аудит
            </Link>
            <Link
              href={pathFor('/cases', locale)}
              className="inline-flex h-11 w-full max-w-[340px] items-center justify-center rounded-md border border-[#2a3e64] bg-[#050b18]/70 px-6 text-[13px] font-extrabold text-white sm:w-auto"
            >
              Смотреть кейсы
            </Link>
          </div>
          <div className="mt-6 flex max-w-[640px] flex-nowrap items-center gap-3 overflow-hidden text-[11px] text-slate-300 sm:gap-5 lg:text-[12px]">
            {heroBenefits.map((item) => (
              <span key={item.label} className="flex shrink-0 items-center gap-2 whitespace-nowrap">
                <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full border border-[#2f7bff]/50 bg-[#0b2b6f]/55 text-[#4f92ff]">
...
function ResultSection() {
  return (
    <section className="px-5 pt-5 sm:px-7">
      <div className="group relative overflow-hidden rounded-md border border-[#24528b]/80 bg-[linear-gradient(135deg,rgba(7,18,37,0.96),rgba(8,27,57,0.78))] p-6 shadow-[inset_0_1px_0_rgba(125,178,255,0.10),0_20px_70px_rgba(0,0,0,0.24)] transition duration-500 hover:border-[#3f86ff]/80 sm:p-7">
        ...
        <h2 className="relative text-[24px] font-extrabold tracking-normal text-white">SEO, которое даёт результат</h2>
        <p className="mt-2 text-[13px] leading-6 text-slate-300">
          Мы не просто продвигаем сайты - мы усиливаем структуру, спрос и путь до заявки.
        </p>
        <div className="relative mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {resultCards.map((card) => (
            <article key={card.title} className="seo-hover-card group/card relative min-h-[116px] overflow-hidden rounded-md border border-[#1f4778] bg-[#08142a]/84 p-4 transition duration-300 hover:-translate-y-1 hover:border-[#4b91ff] hover:shadow-[0_18px_45px_rgba(23,96,255,0.18)]">
              ...
              <h3 className="relative mt-4 text-[14px] font-extrabold text-white">{card.title}</h3>
              <p className="relative mt-2 text-[12px] leading-5 text-slate-300">{card.text}</p>
            </article>
          ))}
        </div>
      </div>
...
function ProcessSection() {
  return (
    <section id="process" className="px-5 pt-5 sm:px-7">
      <div className="seo-hover-surface rounded-md border border-[#163155] bg-[#06101f]/86 p-6 sm:p-7">
        <h2 className="text-[24px] font-extrabold tracking-normal text-white">Что мы делаем</h2>
        <p className="mt-2 text-[13px] leading-6 text-slate-300">Комплексное SEO-продвижение под ключ</p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {processCards.map((card) => (
            <article key={card.title} className="seo-hover-card group relative min-h-[142px] overflow-hidden rounded-md border border-[#17345e] bg-[#08142a]/76 p-4 transition duration-300 hover:-translate-y-1 hover:border-[#4b91ff] hover:bg-[#0a1b39]/90">
    ...
    <section id="services" className="px-5 pt-5 sm:px-7">
      <div className="seo-hover-surface rounded-md border border-[#163155] bg-[#06101f]/86 p-6 sm:p-7">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-[24px] font-extrabold tracking-normal text-white">Услуги</h2>
            <p className="mt-2 text-[13px] leading-6 text-slate-300">Комплексные решения для роста вашего бизнеса</p>
          </div>
...
function CasesSection({ locale }: { locale: Locale }) {
  return (
    <section id="cases" className="px-5 pt-5 sm:px-7">
      <div className="seo-hover-surface rounded-md border border-[#163155] bg-[#071225]/86 p-6 sm:p-7">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-[24px] font-extrabold tracking-normal text-white">Кейсы</h2>
            <p className="mt-2 text-[13px] leading-6 text-slate-300">Реальные проекты и результаты</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {caseCards.map((item) => (
            <Link key={item.title} href={pathFor('/cases', locale)} className="seo-hover-card group relative overflow-hidden rounded-md border border-[#17345e] bg-[#08142a]/80 transition duration-300 hover:-translate-y-1 hover:border-[#4b91ff] hover:shadow-[0_18px_45px_rgba(23,96,255,0.20)]">
              <div className="relative h-32 overflow-hidden border-b border-[#17345e]">
                <Image src={item.image} alt="" fill sizes="360px" className="origin-top object-cover object-top opacity-90 transition duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(3,8,18,0.05),rgba(3,8,18,0.62))]" />
              </div>
              <div className="p-4">
                <p className="text-[11px] uppercase tracking-[0.14em] text-slate-400">{item.subtitle}</p>
                <h3 className="mt-2 text-[14px] font-extrabold text-white">{item.title}</h3>
                <p className="mt-3 text-[24px] font-black text-[#4b91ff]">{item.result}</p>
              </div>
            </Link>
          ))}
        </div>
  ...
  return (
    <section id="blog" className="px-5 pt-5 sm:px-7">
      <div className="seo-hover-surface rounded-md border border-[#163155] bg-[#06101f]/86 p-6 sm:p-7">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-[24px] font-extrabold tracking-normal text-white">Блог</h2>
            <p className="mt-2 text-[13px] leading-6 text-slate-300">Полезные материалы с перелинковкой на статьи</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {blogCards.map((post) => (
            <Link key={post.href} href={pathFor(post.href, locale)} className="seo-hover-card group relative overflow-hidden rounded-md border border-[#17345e] bg-[#08142a]/80 transition duration-300 hover:-translate-y-1 hover:border-[#4b91ff] hover:shadow-[0_18px_45px_rgba(23,96,255,0.20)]">
              <div className="relative h-32 overflow-hidden border-b border-[#17345e]">
                <Image src={post.image} alt="" fill sizes="360px" className="origin-top object-cover object-top opacity-90 transition duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(3,8,18,0.05),rgba(3,8,18,0.68))]" />
              </div>
              <div className="p-4">
                <p className="text-[11px] uppercase tracking-[0.14em] text-slate-400">{post.date}</p>
                <h3 className="mt-2 min-h-[54px] text-[14px] font-extrabold leading-snug text-white">{post.title}</h3>
